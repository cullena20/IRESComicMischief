...
# CULLEN: Multi task defines some weights for each class and a different name
mature_w = 0.1
gory_w = 0.4
slap_w = 0.2
sarcasm_w = 0.2
...
# CULLEN: Above pretty similar, but multi task has some extra stuff to create name for test
...
def train(model, optimizer):
    ...
    batch_emo_deepMoji = []
    batch_mask = []
    # CULLEN: single task is just batch_y
    ...
    for index, i in enumerate(sh_train_set):
        ...
        # Cullen: single tasks doesn't do above but does append to batch_y, pretty sure batch_y is useless
...
# Cullen: Evaluation seems rather different, but this should be okay and we can keep mostly as is
...
# Cullen: I think the only difference in below comes from evaluation code
def training_loop():
