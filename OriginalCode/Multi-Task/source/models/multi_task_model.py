...
# CULLEN: this is the same as the binary case
...
class Bert_Model(nn.Module):
    ...
    def __init__(self):
        ...
        # Cullen: Up to here, same as binary model
    ...
    def forward(self, sentences,mask,image, image_mask, audio, audio_mask):
        ...
        # CULLEN: here we are getting image as modality 1, audio as modality 2, and text (hidden[-1]) as modality 3
        # In binary code (see commented out parts) we use text as modality 2 and image as modality 3
        ...
        # output_image = self.features[13](self.features[6](rnn_img_encoded), hidden[-1], extended_attention_mask) original line instead of above
        output_image = self.features[14](output_image)

        output_image = self.features[13](output_image, hidden[-1], extended_attention_mask)
        #output_image = self.features[13](output_image, self.features[5](rnn_audio_encoded) ,extended_audio_attention_mask) original line instead of above
        ...
        # Cullen: Below is the only real difference with binary method (an different head)
