...
def train_loop(model, optimizer, json_train_path, tasks, scheduler=None, json_val_path=None, training_method="all_at_once", loss_setting="unweighted", task_epochs=None, batch_size=16, num_epochs=1, text_pad_length=500, img_pad_length=36, audio_pad_length=63, shuffle=True, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), name="Test"):
    ...
    # make a different dataset for every task, we don't 
    ...
    if training_method == "one_at_a_time":
        num_epochs = 0
        for num_task_epochs in task_epochs.values():
            num_epochs += num_task_epochs
        tasks = list(task_epochs.keys())
        task_idx = 0
    ...
    for epoch in range(num_epochs): 
        ...
        if training_method == "dynamic_difficulty_sampling":
            ...
            if training_method == "one_at_a_time":
                task = [tasks[task_idx]] # make sure that even individual tasks are wrapped as lists
                task_epochs[task_idx] -= 1
                if task_epochs[task_idx] == 0:
                    task_idx += 1
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if training_method == "all_at_once" or training_method == "one_at_a_time":
                ...
                if loss_setting == "gradnorm" and steps == 0: # initialize task losses and optimizer only in the first training step
                    ...
                    gradnorm_optimizer = torch.optim.Adam([loss_weights], lr=0.01)
    
                # Weight losses appropriately
                # given a dictionary of losses for every task, handle the loss reweigthing
                loss, _ = handle_losses(task_losses, loss_setting, loss_weights, steps, tasks, initial_task_losses=initial_task_losses, loss_optimizer=gradnorm_optimizer, model=model)
                ...
                if loss_setting == "gradnorm":
                    # placement is awkward - .data important to know for future
                    with torch.no_grad():
                        loss_weights.data = loss_weights.data / loss_weights.data.sum() * len(tasks)
                ...
                # added this in case it is an issue, made 10 instead of 1 in case
                max_grad_norm = 10.0  # Adjust this value as needed
...
def handle_losses(task_losses, loss_setting, loss_weights, steps, tasks, initial_task_losses=None, loss_optimizer=None, model=None):
    ...
    # this actually doesn't make sense as it would just go to zero
    ...
    # TO DO: Also return method specific parameters - in this case loss weight history
        ...
        _ = gradnorm(task_losses, initial_task_losses, model.base_model, loss_optimizer, loss_weights) # update model weights using grad norm
